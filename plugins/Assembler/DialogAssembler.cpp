			...
			if(exit_code != 0) {
				...
				if(bytes.size() <= instruction_size_) {
					if(ui->fillWithNOPs->isChecked()) {
                        ...
                        edb::v1::modify_bytes(address_, instruction_size_, bytes, 0x90,true);
					} else {
                        edb::v1::modify_bytes(address_, instruction_size_, bytes, 0x00,true);
					}
				} else {
					if(ui->keepSize->isChecked()) {
						QMessageBox::warning(this, tr("Error In Code"), tr("New instruction is too big to fit."));
					} else {
                        edb::v1::modify_bytes(address_, bytes.size(), bytes, 0x00,true);
					}
				}
			}
		}
